        ...
        <div className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3 translate-x-20 md:translate-x-0">
          {heroSlides.map((_, index) => (
            <button
              ...
              className={`w-1 transition-all duration-500 ${index === currentSlide
                  ? "h-12 bg-neon"
            ...
            <div className="lg:pl-6">
              ...
              <div className="space-y-0 mb-10">
                {storyFeatures.map((feature, index) => (
                  <div
                    key={index}
                    className={`border-t border-[#ddd] ${index === storyFeatures.length - 1 ? 'border-b' : ''} transition-all duration-300 ${activeAccordion === index ? 'bg-gradient-to-r from-neon/5 to-transparent' : 'hover:bg-[#f0efe8]/50'
                      }`}
                  >
                    <button
                    ...
                    >
                      ...
                      <span className={`text-2xl font-light ml-4 flex-shrink-0 transition-transform duration-300 ${activeAccordion === index ? 'text-neon rotate-0' : 'text-[#1a1a1a]'
                        }`}>
                    ...
                    {/* Expandable Content */}
                    <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeAccordion === index ? 'max-h-40 opacity-100 pb-5' : 'max-h-0 opacity-0'
                      }`}>
