      ...
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-muted-foreground mt-1">
          {search || statusFilter !== 'all' || typeFilter !== 'all' || directionFilter !== 'all'
            ...
            : isStaffTab ? 'No staff-created DCs yet' : 'Create your first Delivery Challan to get started'}
        </p>
      </div>
    );
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/20 hover:bg-muted/20">
            ...
            {isStaffTab && <TableHead className="font-semibold">Created By</TableHead>}
          ...
          {filteredChallans.map((dc) => {
            ...
            return (
              <TableRow key={dc.id} className="group hover:bg-muted/50 transition-colors">
                <TableCell className="font-mono font-semibold text-primary">{dc.dc_number}</TableCell>
                <TableCell className="text-muted-foreground">{format(new Date(dc.dc_date), 'dd MMM yyyy')}</TableCell>
                <TableCell>
                  <Badge variant="outline" className={directionColors[direction]}>
                    {direction === 'given' ? <><ArrowUpRight className="h-3 w-3 mr-1" /> Given</> : <><ArrowDownLeft className="h-3 w-3 mr-1" /> Taken</>}
                  </Badge>
                </TableCell>
                <TableCell className="font-medium max-w-[200px] truncate">{dc.job_worker_name}</TableCell>
                {isStaffTab && (
                  <TableCell className="text-sm text-muted-foreground">{dc.created_by_employee_code || '—'}</TableCell>
                )}
                <TableCell>
                  <Badge variant="outline" className={dcTypeColors[dc.dc_type]}>{DC_TYPE_LABELS[dc.dc_type].split(' ')[0]}</Badge>
                </TableCell>
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button variant="ghost" size="icon" onClick={() => onView(dc.id)} className="h-8 w-8"><Eye className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" onClick={() => onPrint(dc.id)} className="h-8 w-8"><Printer className="h-4 w-4" /></Button>
                    {!isStaffTab && dc.status === 'created' && (
                      <Button variant="ghost" size="icon" onClick={() => onEdit(dc.id)} className="h-8 w-8"><Edit className="h-4 w-4" /></Button>
                    )}
                    {!isStaffTab && (
                      <Button variant="ghost" size="icon" onClick={() => onDelete(dc.id)} className="h-8 w-8 text-destructive hover:text-destructive"><Trash2 className="h-4 w-4" /></Button>
                    )}
                  </div>
                </TableCell>
              </TableRow>
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary via-primary/95 to-primary/90 p-8 text-primary-foreground">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <Button onClick={() => navigate('/admin/delivery-challan/create')} size="lg" className="bg-white text-primary hover:bg-white/90 shadow-lg gap-2">
            <Plus className="h-5 w-5" /> Create New DC
          </Button>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); setSearch(''); setStatusFilter('all'); setTypeFilter('all'); setDirectionFilter('all'); }}>
        <TabsList>
          <TabsTrigger value="our-dcs" className="gap-2">
            <Truck className="h-4 w-4" /> Our DCs ({adminChallans.length})
          </TabsTrigger>
          <TabsTrigger value="staff-dcs" className="gap-2">
            <Users className="h-4 w-4" /> Staff DCs ({staffChallans.length})
          </TabsTrigger>
        </TabsList>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input placeholder="Search by DC No, Job Worker, or Vehicle..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 h-11" />
                </div>
                <div className="flex gap-3 flex-wrap">
                  <Select value={directionFilter} onValueChange={setDirectionFilter}>
                    <SelectTrigger className="w-[160px] h-11"><Filter className="h-4 w-4 mr-2" /><SelectValue placeholder="Direction" /></SelectTrigger>
                    <SelectContent>
                  ...
                  <Select value={typeFilter} onValueChange={setTypeFilter}>
                    <SelectTrigger className="w-[160px] h-11"><Filter className="h-4 w-4 mr-2" /><SelectValue placeholder="DC Type" /></SelectTrigger>
                    <SelectContent>
                  ...
                  <Select value={statusFilter} onValueChange={setStatusFilter}>
                    <SelectTrigger className="w-[160px] h-11"><Filter className="h-4 w-4 mr-2" /><SelectValue placeholder="Status" /></SelectTrigger>
                    <SelectContent>
          ...
          <Card className="border-0 shadow-sm overflow-hidden">
            <CardHeader className="bg-muted/30 border-b px-6 py-4">
              <CardTitle className="text-lg font-semibold">
                {activeTab === 'our-dcs' ? 'Delivery Challans' : 'Staff-Created Delivery Challans'}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <TabsContent value="our-dcs" className="mt-0">
                <DCTable
                  challans={adminChallans}
                  ...
                  isStaffTab={false}
                  onView={(id) => navigate(`/admin/delivery-challan/${id}`)}
                  onPrint={(id) => navigate(`/admin/delivery-challan/print/${id}`)}
                  onEdit={(id) => navigate(`/admin/delivery-challan/edit/${id}`)}
                  onDelete={(id) => setDeleteId(id)}
                />
              </TabsContent>
              <TabsContent value="staff-dcs" className="mt-0">
                <DCTable
                  challans={staffChallans}
